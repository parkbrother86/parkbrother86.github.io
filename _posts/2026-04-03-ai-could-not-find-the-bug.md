...
<div class="bilingual">
  <p class="en lede">AI coding tools are very good at producing code that looks right. They are also surprisingly good at review. What they still cannot tell me is how a live server will behave once thousands of sessions start colliding in real time. This post is about how I ended up finding the real bugs only after 6,216 simulation runs and more than 100 load tests.</p>
  <p class="ko lede">AI 코딩 도구는 겉으로 맞는 코드를 만드는 데 정말 강하다. 코드 리뷰도 꽤 잘한다. 하지만 수천 개의 세션이 실제 런타임에서 동시에 부딪히기 시작할 때 서버가 어떻게 무너지는지는 말해주지 못한다. 이 글은 내가 그 진짜 버그를 6,216회의 시뮬레이션과 100회가 넘는 부하 실험 끝에야 찾아낸 기록이다.</p>
</div>
...
<div class="bilingual">
  <p class="en">I am building an MMORPG server on top of Nakama, with Go plugins carrying the game-specific logic. Most of my coding flow goes through tools like Claude Code and Codex CLI. They help write code, review it, and clean it up. If I had to describe the feeling, one AI session feels like a competent senior driving the task with several fast juniors implementing around it.</p>
  <p class="ko">나는 지금 Nakama 위에 Go 플러그인을 얹는 구조로 MMORPG 서버를 만들고 있다. 개발 과정의 대부분은 Claude Code나 Codex CLI 같은 도구와 함께 간다. 코드를 쓰고, 리뷰하고, 정리하는 데 정말 큰 도움이 된다. 체감상으로는 꽤 괜찮은 시니어 한 명이 방향을 잡고, 손 빠른 주니어 몇 명이 주변 구현을 계속 붙여주는 느낌에 가깝다.</p>
</div>

<div class="bilingual">
  <p class="en">In a local sense, AI is excellent. Type checks pass. Tests pass. Small edge cases get caught. The problem is that local correctness is not the same thing as system behavior. A diff can look clean while Redis state, presence streams, WebSocket lifecycles, retries, and load spikes are quietly setting up a mess.</p>
  <p class="ko">로컬한 관점에서 보면 AI는 정말 훌륭하다. 타입 체크도 통과하고, 테스트도 통과하고, 자잘한 엣지 케이스도 잘 잡아낸다. 문제는 로컬한 정확성이 곧 시스템 동작을 뜻하지는 않는다는 점이다. diff 상으로는 깔끔해 보여도 Redis 상태, presence stream, WebSocket lifecycle, retry, 순간적인 부하 급증이 함께 얽히기 시작하면 서버는 아주 지저분한 방식으로 무너질 수 있다.</p>
</div>
